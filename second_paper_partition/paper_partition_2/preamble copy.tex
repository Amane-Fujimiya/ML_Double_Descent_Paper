...
\usepackage{graphicx}
\usepackage[a4paper,margin=1in,twoside=false]{geometry} % configure the geometry page
...
\titleformat{\section}[hang]            % shape: hang, block, display, runin, leftmargin, rightmargin
  {\large}                     % format applied to whole title (font + size)
  {\thesection}                         % label (what prints the number)
  {1em}                                 % separation between label and title text
  {}                                    % code before title text
  []                                    % code after title text (optional)
