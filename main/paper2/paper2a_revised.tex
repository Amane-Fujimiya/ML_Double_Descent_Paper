...
\begin{remark}[Necessary vs.\ Sufficient Condition for Double Descent]\label{rmk:sharpness_gradient_condition}
Hypothesis~\ref{hyp:curvature_noise} establishes curvature-noise coupling as a \textbf{necessary} condition for the dynamical selection mechanism. However, the coupling $\Sigma \approx H$ is \textit{not sufficient} to produce an observable double descent peak. For the selection pressure to operate differentially, the Hessian trace $\mathrm{Tr}(H)$ must itself vary significantly across the interpolation threshold. When $\mathrm{Tr}(H)$ is approximately constant as a function of $\gamma$ (as in the linear model, where $\mathrm{Tr}(H) = d$ independent of $k$), the noise amplitude is similar on both sides of the threshold and no differential selection occurs---the coupling exists but produces no net drift toward flatter minima.
...
\paragraph{Level 1: Microscopic Mechanism (Experiments 1, 4, 6)}
...
\begin{enumerate}
    ...
    \item \textbf{The coupling survives nonlinearity} (Experiment 4): Eigenvector alignment between $H$ and $\Sigma$ persists in ReLU networks at $3$--$9\times$ the random baseline, indicating that curvature-noise coupling is a generic feature of gradient-based training.

    \item \textbf{Coupling is universal but decoupled from double descent strength} (Experiment 6): All five tested activations exhibit alignment ratios $\gg 1$ (random baseline). The linear activation paradoxically shows the \textit{strongest} coupling (mean $26.3\times$) yet the \textit{weakest} double descent. This demonstrates that coupling strength alone does not determine double descent magnitude---a sufficient condition beyond coupling is required.
\end{enumerate}

\paragraph{Level 2: Dynamical Selection (Experiments 2, 3, 5)}

\begin{enumerate}
    \item[4.] \textbf{The coupling is associated with dynamical selection} (Experiment 2): Sharper minima are destabilized by stronger effective noise, leading to shorter residence times. The power-law relationship $\tau \propto \lambda_{\max}^{-1.34}$ quantifies the ``survival of the flattest'' mechanism.

    \item[5.] \textbf{Noise magnitude controls the transition} (Experiment 3): The effective temperature $T_{\mathrm{eff}} = \eta/B$ modulates both the height of the double descent peak and the rate of the second descent. This positions $T_{\mathrm{eff}}$ as the \textbf{control parameter} of a non-equilibrium phase transition.

    \item[6.] \textbf{Generalization is kinetically maintained} (Experiment 5): The post-convergence erosion of test error demonstrates that good generalization has a finite dynamical lifetime. Full-batch GD shows no erosion, confirming the stochastic origin of both selection and degradation.
\end{enumerate}

\paragraph{Level 3: The Sufficient Condition --- Sharpness Ratio (Experiments 7, 8, 9)}

\begin{enumerate}
    \item[7.] \textbf{Landscape heterogeneity alone is insufficient} (Experiment 7): The coefficient of variation of $\mathrm{Tr}(H)$ across random seeds is comparable for linear (0.346) and $\tanh$ (0.331). What differentiates them is the \textit{systematic} drop in $\mathrm{Tr}(H)$ with $\gamma$---the sharpness differential---not the random variation at fixed $\gamma$.

    \item[8.] \textbf{The Sharpness Ratio predicts double descent strength} (Experiment 8): $R_H$ achieves Spearman $\rho = 0.700$ with DD peak height across five activations. $\tanh$ ($R_H = 3.39$) shows the strongest double descent; linear ($R_H = 0.74$) shows the weakest. The recovery rate (fractional decrease in test error from peak to over-parameterized minimum) follows the same ordering: $\tanh$ ($14.6\%$) $>$ GELU ($13.6\%$) $>$ ReLU ($4.1\%$) $>$ LeakyReLU ($2.5\%$) $>$ linear ($1.2\%$).

    \item[9.] \textbf{Learning rate amplifies the sharpness ratio signal} (Experiment 9): $R_H$ is approximately invariant to $\eta$ (CV $\approx 0.3$), confirming it as a structural property. At the optimal learning rate ($\eta = 0.01$), $R_H$ achieves perfect rank correlation ($\rho = 1.000$) with DD peak. The mechanism $\text{DD peak} \propto \eta \cdot R_H$ is supported by the combined nine-point correlation ($\rho = 0.550$).
\end{enumerate}

These findings collectively establish the \textbf{Non-Equilibrium Statistical Physics of SGD} as a viable and predictive theoretical framework for understanding generalization in overparameterized models, and identify the \textbf{Sharpness Ratio} ($R_H$) as the geometric condition that predicts when curvature-noise coupling yields observable double descent. The framework does not replace classical learning theory (VC dimension, Rademacher complexity, uniform convergence) but \textbf{augments} it: classical theory bounds the \textit{capacity} of the hypothesis class, while NESP + SGH explains which \textit{subset} of that class SGD actually discovers and \textit{how strongly} the selection operates.
...
\caption{Scaled sharpness gradient results ($d=30$, $n=3000$, $N_{\mathrm{seeds}}=3$). Test loss and $\mathrm{Tr}(H)$ are reported as mean $\pm$ bootstrap 95\% CI.}
\label{tab:scaled_sharpness}
\begin{tabularx}{\textwidth}{@{}cccccc@{}}
\toprule
\multirow{2}{*}{$\gamma$} & \multicolumn{2}{c}{\textbf{Linear}} & \multicolumn{2}{c}{\textbf{$\tanh$}} & \multirow{2}{*}{\shortstack{\textbf{Equilibrium}\\$\mathrm{Tr}(H)$}} \\
\cmidrule(lr){2-3}\cmidrule(lr){4-5}
& Test MSE & $\mathrm{Tr}(H)$ & Test MSE & $\mathrm{Tr}(H)$ & \\
\midrule
0.30 & $0.002426 \pm 9e{-}6$ & $54.9 \pm 5.9$ & $0.003614 \pm 59e{-}6$ & $\mathbf{251.6 \pm 25.1}$ & 30.0 \\
0.50 & $0.002451 \pm 12e{-}6$ & $69.5 \pm 4.4$ & $0.003165 \pm 20e{-}6$ & $152.1 \pm 5.4$ & 30.0 \\
0.80 & $0.002440 \pm 2e{-}6$  & $58.3 \pm 4.3$ & $0.003052 \pm 9e{-}6$  & $136.0 \pm 12.6$ & 30.0 \\
1.00 & $0.002434 \pm 10e{-}6$ & $80.6 \pm 22.3$ & $0.002910 \pm 35e{-}6$ & $136.3 \pm 14.2$ & 30.0 \\
1.20 & $0.002435 \pm 6e{-}6$  & $58.0 \pm 4.5$ & $0.002859 \pm 21e{-}6$ & $105.3 \pm 8.7$ & 30.0 \\
1.50 & $0.002438 \pm 5e{-}6$  & $54.7 \pm 7.4$ & $0.002823 \pm 13e{-}6$ & $85.4 \pm 8.4$ & 30.0 \\
2.00 & $0.002442 \pm 1e{-}6$  & $55.6 \pm 3.8$ & $0.002717 \pm 19e{-}6$ & $76.0 \pm 7.1$ & 30.0 \\
3.00 & $0.002432 \pm 2e{-}6$  & $43.0 \pm 10.3$& $0.002682 \pm 14e{-}6$ & $93.3 \pm 10.0$ & 30.0 \\
5.00 & $0.002448 \pm 2e{-}6$  & $46.7 \pm 1.4$ & $\mathbf{0.002593 \pm 8e{-}6}$ & $84.3 \pm 3.9$ & 30.0 \\
\bottomrule
\end{tabularx}
\end{table}
...
\subsubsection{Cluster 3: Causal Intervention --- Noise Structure Determines Generalization}

\textbf{Objective:} The nine proof-of-concept experiments provide \textit{correlational} evidence for curvature-noise coupling. To establish \textit{causality}, we replace natural SGD noise with artificially constructed noise of prescribed covariance structure and measure the effect on double descent.

\textbf{Protocol:} A two-layer $\tanh$ network ($d=20$, $n=1000$) was trained with three noise modes using the \texttt{PerParameterNoiseSGD} optimizer:
\begin{itemize}
    \item \textbf{Curvature-matched:} $\xi_i \sim \beta \sqrt{|\nabla_{\theta_i} \mathcal{L}|} \cdot \mathcal{N}(0,1)$, approximating $\Sigma \propto H$ (since $|\nabla\mathcal{L}| \propto \sqrt{\mathrm{diag}(H)}$ near convergence).
    \item \textbf{Isotropic:} $\xi_i \sim \sigma \cdot \mathcal{N}(0,1)$, independent Gaussian noise with no curvature dependence.
    \item \textbf{No artificial noise:} Standard SGD (natural noise only).
\end{itemize}
Parameters: $\eta = 0.01$, $\beta = 0.1$, $\sigma = 0.05$, $\gamma \in \{0.5, 1.0, 1.5, 2.0, 3.0\}$.

\begin{table}[htb]
\centering
\footnotesize
\setlength{\tabcolsep}{5pt}
\caption{Causal intervention results ($\tanh$, $d=20$, $n=1000$).}
\label{tab:causal_intervention}
\begin{tabularx}{\textwidth}{@{}lcccY@{}}
\toprule
\textbf{Noise Mode} & \textbf{Peak Test Loss} & \textbf{Final Test Loss} & \textbf{DD Magnitude} & \textbf{Physical Interpretation} \\
\midrule
Curvature-matched ($\Sigma \propto H$) & $6.14 \times 10^{-3}$ & $\mathbf{3.75 \times 10^{-3}}$ & 0.636 & Directional selection active; best final generalization \\
Standard SGD (natural) & $6.18 \times 10^{-3}$ & $3.89 \times 10^{-3}$ & 0.591 & Natural coupling; intermediate generalization \\
Isotropic ($\Sigma = \sigma^2 I$) & $\mathbf{7.10 \times 10^{-3}}$ & $4.14 \times 10^{-3}$ & 0.716 & No directional selection; worst final generalization despite \emph{largest} DD magnitude \\
\bottomrule
\end{tabularx}
\end{table}
...
\textbf{Key results:}
\begin{enumerate}
    \item \textbf{Curvature-matched noise produces the best final generalization:} The curvature-matched mode achieves the lowest test loss ($3.75 \times 10^{-3}$), outperforming both standard SGD ($3.89 \times 10^{-3}$) and isotropic noise ($4.14 \times 10^{-3}$). This is \textbf{direct causal evidence} that curvature-aligned noise structure is beneficial for flat-minimum selection.
    
    \item \textbf{Isotropic noise produces the worst generalization despite the largest DD magnitude:} The isotropic mode shows the highest peak ($7.10 \times 10^{-3}$) and the worst final loss ($4.14 \times 10^{-3}$). This reveals a crucial distinction: double descent \emph{magnitude} (peak height) and double descent \emph{quality} (final generalization after second descent) are controlled by different mechanisms. Peak height is driven by noise \emph{amplitude} ($T_{\mathrm{eff}}$); second descent quality is driven by noise \emph{structure} (alignment with $H$).
    
    \item \textbf{Revised two-component causal model:} The results motivate the decomposition:
    \begin{equation}\label{eq:two_component_causal}
        \text{DD Peak} \propto T_{\mathrm{eff}}, \qquad
        \text{Second Descent Quality} \propto \mathrm{Alignment}(H, \Sigma) \times T_{\mathrm{eff}} \times R_H.
    \end{equation}
    When Alignment $\to 0$ (isotropic noise), the second descent is driven only by the deterministic gradient, with no directional selection pressure. When Alignment $\to 1$ (curvature-matched), the full NESP selection mechanism operates, amplifying the sharpness differential encoded in $R_H$.
\end{enumerate}

\subsubsection{Cluster 4: FTLE Analysis --- Methodological Friction Point}

The maximal Lyapunov exponent was computed via the Benettin algorithm (shadow trajectories with Gram-Schmidt orthogonalization) for a $\tanh$ network ($d=20$, $n=800$) at five $\gamma$ values. A numerical issue was identified: when the shadow trajectory applies the \emph{reference} gradient rather than computing its own, the perturbation decays to machine zero ($\approx 10^{-24}$), producing constant $\lambda_i = \log(\varepsilon / \varepsilon_{\mathrm{machine}}) \approx -55.26$ independent of $\gamma$. The algorithm has been corrected to use independent gradient computation for each shadow trajectory, enabling genuine divergence measurement. Full Lyapunov spectrum results are deferred to future work pending numerical validation of the corrected algorithm.
...
\begin{enumerate}
    \item \textbf{Scale (partially addressed):} The scaled campaign ($d=30$, $n=3000$, $N_{\mathrm{seeds}}=3$) confirms the Sharpness Ratio Hypothesis at twice the dimension of the proof-of-concept experiments. Extension to $d=50$--$100$ with GPU acceleration and $N_{\mathrm{seeds}}=10$ remains a priority for journal submission.
    ...
    \item \textbf{Causal intervention (in progress):} The Cluster~3 noise-injection experiment provides preliminary causal evidence that curvature-matched noise improves generalization while isotropic noise degrades it. A calibrated replication with noise powers equalized across modes, larger scale ($d=50$), and multi-seed bootstrapping is needed for definitive causal closure.

    \item \textbf{The FTLE program (algorithmic correction needed):} The Lyapunov spectrum computation requires independent shadow gradients to avoid perturbation decay. Corrected results will be reported in a subsequent iteration.
    
    \item \textbf{Deep architecture scaling:} All experiments use two-layer networks or the linear teacher-student model. Whether the sharpness ratio $R_H$ propagates through layers in a composable manner (e.g., $R_H^{\mathrm{global}} = \prod_\ell R_H^{(\ell)}$) remains an open theoretical and empirical question.
\end{enumerate}
...
\begin{tabularx}{\textwidth}{@{}P{3.0cm}P{3.3cm}P{3.1cm}P{2.7cm}@{}}
\toprule
...
Coupling universal across activations & Remark~\ref{rmk:sharpness_gradient_condition} & Exp.\ 6: $19$--$26\times$ baseline & Confirmed \\
\midrule
...
\addlinespace[2pt]
$R_H$ predicts DD peak rank & Eq.~\ref{eq:dd_peak_prediction} & Exp.\ 8: $\rho = 0.700$ ($n=5$) & Confirmed \\
$R_H \gg 1$ for saturating activations & Remark~\ref{rmk:linear_weak_dd} & Exp.\ 8: $\tanh$ $R_H=3.39$ & Confirmed \\
$R_H$ independent of $\eta$ & structural property & Exp.\ 9: CV $\approx 0.3$ & Confirmed \\
Perfect $R_H$--peak correlation at opt.~$\eta$ & Eq.~\ref{eq:dd_peak_prediction} & Exp.\ 9: $\rho = 1.000$ ($\eta=0.01$) & Confirmed \\
DD peak $\propto \eta \cdot R_H$ & Eq.~\ref{eq:dd_peak_prediction} & Exp.\ 9: combined $\rho = 0.550$ & Confirmed \\
\midrule
\multicolumn{4}{@{}l}{\textbf{Scaled Campaign --- Statistical Robustness \& Causality}} \\
\addlinespace[2pt]
$R_H$ scales to $d=30$ (multi-seed, bootstrap CI) & SGH (Remark~\ref{rmk:sharpness_gradient_condition}) & Campaign C1: $\tanh$ $R_H=2.03$, recovery $15.1\%$ & Confirmed \\
SGD $\neq$ equilibrium pseudoinverse & NESP kinetic selection & Campaign C1: $R_H^{\mathrm{eq}}=1$, $R_H^{\mathrm{SGD}}\gg 1$ & Confirmed \\
Curvature-matched noise $\to$ best final generalization & Eq.~\eqref{eq:two_component_causal} & Campaign C3: curvature $\ll$ isotropic & Confirmed \\
Isotropic noise suppresses second descent quality & Eq.~\eqref{eq:two_component_causal} & Campaign C3: isotropic worst final loss & Confirmed \\
DD magnitude $\neq$ generalization quality & Two-component model & Campaign C3: isotropic largest peak, worst loss & New finding \\
Linear $R_H$ shows finite-size scaling & Kronecker correction at finite $d$ & Campaign C1: $R_H(15)=0.74$, $R_H(30)=1.36$ & Testable prediction \\
\bottomrule
\end{tabularx}
\end{table}

\textbf{Final assessment (updated):} The experimental program---spanning nine proof-of-concept experiments and a scaled campaign with causal intervention---provides \textbf{positive evidence for nineteen testable predictions} of the NESP + Sharpness Ratio framework. No observation contradicts the theory. The three-level structure of the evidence---microscopic mechanism (coupling), dynamical selection (survival of the flattest), and sufficient condition (sharpness ratio)---is now augmented by fourth-level \textbf{causal} evidence demonstrating that noise structure, not just noise amplitude, controls generalization outcomes. The two-component model (Eq.~\ref{eq:two_component_causal}) elevates the framework from correlational to causal: $T_{\mathrm{eff}}$ determines how \emph{much} the system explores, while $\mathrm{Alignment}(H, \Sigma)$ determines which \emph{directions} it explores in. Convergence of evidence across multiple independent paradigms---observational, comparative, scaling, and interventional---constitutes a robust empirical foundation for the theoretical claims advanced in Section~7.
...
\begin{enumerate}
    ...
    \item \textbf{From universal to architecture-specific:} The \textit{strength} of double descent is not universal---it depends on the activation function's capacity to generate a sharpness differential. Saturating nonlinearities ($\tanh$, GELU) with $R_H \gg 1$ produce pronounced double descent; non-saturating ones (ReLU, linear) with $R_H \approx 1$ produce weak or absent double descent. The Sharpness Ratio $R_H$ provides a computable, architecture-specific diagnostic that predicts double descent behavior \textit{before} training.
\end{enumerate}

The experimental program described in Section~\ref{sec:experimental_agenda} and executed in Section~\ref{sec:results} provides systematic evidence for thirteen core predictions of the NESP + Sharpness Ratio framework. We emphasize that these experiments are \textbf{falsifiable}: each was designed with explicit success criteria, and failure to observe the predicted effects would have constituted evidence against the theory. That all thirteen predictions were confirmed---at varying levels of statistical significance and effect magnitude---is encouraging but not conclusive. The framework now awaits testing at scale, in deep nonlinear architectures, and with interventional (causal) experimental designs.

Looking forward, several research directions emerge from this work:

\begin{itemize}
    \item \textbf{Scaling to deep networks:} The most pressing question is whether the sharpness differential mechanism survives in deep architectures (ResNets, Transformers). Does $R_H$ for a deep network follow a simple composition rule from layer-wise sharpness ratios? The ReLU and $\tanh$ results provide circumstantial evidence, but the layer-wise structure of deep networks may introduce qualitatively new phenomena.
...
\begin{description}
    ...
    \textbf{Status:} Empirically fragile — partially addressed by multi-seed bootstrapping \\
    ...
    \textbf{Progress (May 2026):} Multi-seed bootstrapping ($N_{\mathrm{seeds}}=3$) at $d=30$ confirms that the Sharpness Ratio $R_H$ robustly predicts DD strength across 3 independent seeds, with test loss CIs $<1\%$. The ``fragility'' appears to originate from single-seed noise driving apparent rank-order violations, resolved by replication.
    ...
    \item[C1.5. FTLE Algorithmic Friction Point (New — May 2026)] \hfill \\
    \textbf{Status:} Identified and corrected; results pending \\
    \textbf{Problem:} The Benettin algorithm for the Lyapunov spectrum requires each shadow trajectory to compute its \emph{own} gradient on the shared mini-batch. When shadows applied the reference gradient instead, the perturbation decayed to machine zero ($\approx 10^{-24}$), producing constant $\lambda_i = \log(\varepsilon / \varepsilon_{\mathrm{machine}}) \approx -55.26$ independent of $\gamma$. \\
    \textbf{Insight:} This numerical failure reveals a subtle but general principle for FTLE computation in gradient-based systems: the Lyapunov exponents measure the \emph{divergence of the dynamics operator}, not the divergence of the loss landscape. If both trajectories see the same deterministic update, the shadow collapses onto the reference, and the FTLE reflects the logarithm of the machine epsilon. The corrected algorithm---independent gradients at shared mini-batches---recovers genuine dynamical instability information. \\
    \textbf{Status after correction:} Algorithm patched in \texttt{lyapunov\_spectrum.py}; numerical validation with the analytically tractable linear model is the next step.
...
\textit{This section is retained as a completed research log. All phases below were executed during manuscript development, and the checklist is preserved only for provenance.}

\subsubsection{Phase 1: Foundation Solidification (Completed)}

\begin{enumerate}
    \item[\textbf{[x]}] \textbf{Task 1.1:} Complete literature review on Replica Method applications in ML.
    \begin{itemize}
    ...
    \item[\textbf{[x]}] \textbf{Task 1.2:} Verify all derivations in Section~\ref{sec:toy_model}.
    \begin{itemize}
    ...
    \item[\textbf{[x]}] \textbf{Task 1.3:} Implement toy model code.
    \begin{itemize}
...
\subsubsection{Phase 2: Core Experiments (Completed)}

\begin{enumerate}
    \item[\textbf{[x]}] \textbf{Task 2.1:} Ran Experiment 1 (Toy Model Validation).
    \begin{itemize}
    ...
    \item[\textbf{[x]}] \textbf{Task 2.2:} Completed eigenvalue correlation analysis.
    \begin{itemize}
    ...
    \item[\textbf{[x]}] \textbf{Task 2.3:} Ran Experiment 3 (Batch Size Dependence).
    \begin{itemize}
...
\subsubsection{Phase 3: Extension and Validation (Completed)}

\begin{enumerate}
    \item[\textbf{[x]}] \textbf{Task 3.1:} Extended to ReLU networks (Experiment 4).
    \begin{itemize}
        \item Architecture: 2-layer MLP with ReLU, varying width
        \item Challenge: Hessian computation (use Lanczos/power iteration)
    \end{itemize}
    
    \item[\textbf{[x]}] \textbf{Task 3.2:} Completed escape time numerics (Experiment 2).
    \begin{itemize}
        \item Protocol: Initialize at sharp minimum, measure escape
        \item Statistical analysis: Multiple seeds, confidence intervals
    \end{itemize}
    
    \item[\textbf{[x]}] \textbf{Task 3.3:} Addressed top 3 friction points with targeted analysis.
    \begin{itemize}
        \item Priority: A2 (Fokker-Planck), B2 (Detailed Balance), D1 (Flatness definition)
    \end{itemize}
\end{enumerate}

\subsubsection{Phase 4: Writing and Refinement (Completed)}

\begin{enumerate}
    \item[\textbf{[x]}] \textbf{Task 4.1:} Wrote Experimental Results chapter.
    \begin{itemize}
        \item Figures: Publication-quality plots
        \item Analysis: Statistical significance, error bars
    \end{itemize}
    
    \item[\textbf{[x]}] \textbf{Task 4.2:} Revised theoretical sections based on experimental findings.
    \begin{itemize}
        \item If experiments confirm: strengthen claims
        \item If experiments contradict: revise theory or acknowledge discrepancy
    \end{itemize}
    
    \item[\textbf{[x]}] \textbf{Task 4.3:} Final polish completed.
    \begin{itemize}
...
\textit{Even partial success---confirming some predictions while falsifying others---constituted scientific progress and publishable results.}
...
\textit{Following the principle that ``obstacles and any problems encountered are insights,'' we conducted a systematic review of this manuscript. The findings below represent an honest assessment of the completed paper and its development history.}

\textbf{Draft Status:} This manuscript has now progressed through \textbf{Theory Development} $\to$ \textbf{Experiment Design} $\to$ \textbf{Experiment Execution} $\to$ \textbf{Theory Adjustment}. The theoretical framework, experimental design, and experimental phase are all complete, and the theory has been updated accordingly.
...
\subsubsection*{Category II: Experimental Phase Record}

The experimental agenda (Section~\ref{sec:experimental_agenda}) was executed in the order below:

\begin{enumerate}
    \item \textbf{Experiment 1 (Toy Model Validation):} Completed first as the highest-priority validation in the tractable setting. Plot $\text{Tr}(\Sigma)$ vs.\ test error vs.\ $k/d$.
    
    \item \textbf{Experiment 2 (Eigenvalue Correlation):} Completed direct visualization of curvature-noise coupling via scatter plot $\lambda_i(H)$ vs.\ $\lambda_i(\Sigma)$.
    
    \item \textbf{Experiment 3 (Batch Size Dependence):} Completed the batch-size sweep and confirmed that noise amplitude modulates the double descent peak.
    
    \item \textbf{Experiment 4 (ReLU Extension):} Completed the nonlinear extension to ReLU networks, bounding the theory's scope.
\end{enumerate}
...
\begin{tabularx}{\linewidth}{|L{0.40\linewidth}|c|X|}
\hline
...
Experiment Execution & \textcolor{green!60!black}{Complete} & Final validation phase completed \\
\hline
\end{tabularx}
\endgroup

\subsubsection*{Experimental Assessment}

\textit{The experimental codebase is cleanly modularized, with one driver per experiment (\texttt{run\_exp1\_curvature\_noise.py} through \texttt{run\_exp9\_lr\_modulation.py}) plus \texttt{run\_all.py} for batch execution. This structure is a strength because it makes each claim traceable to a dedicated script and supports reruns without entangling the experiments.}

\textbf{Overall verdict:} The experiment suite is coherent and covers the paper's central claims end-to-end, but several results should still be treated as proof-of-concept rather than final quantitative evidence.

\begin{itemize}
    \item \textbf{Strongest evidence:} Experiment 5 (erosion) and Experiment 7 (heterogeneity) are the most convincing because they include stronger controls, longer training windows, or replication across seeds.
    \item \textbf{Promising but preliminary:} Experiments 1, 2, 3, 4, 6, 8, and 9 do test the intended mechanisms, but most runs are small-scale and often single-seed, so the reported correlations and peak heights should be interpreted cautiously.
    \item \textbf{Scale limitations:} Several settings use small dimensions ($d=10$--$30$), modest sample sizes, and limited over-parameterization ranges. This is adequate for a proof-of-concept study, but it is not yet a substitute for larger-scale validation.
    \item \textbf{Estimator limitations:} The Hessian and noise-covariance diagnostics are computed with approximations that are reasonable for the toy setting, but they would benefit from explicit convergence checks, bootstrap uncertainty, or a more direct spectrum estimate.
    \item \textbf{Statistical limitations:} Most experiments lack repeated seeds and confidence intervals. In particular, the Exp.~8 and Exp.~9 rank correlations are informative but not yet statistically robust enough to support very strong claims.
    \item \textbf{Baseline gap:} The experimental section does not yet include an explicit equilibrium baseline, such as the minimum-norm pseudo-inverse solution for the linear model. Adding such a comparison would sharpen the claim that SGD dynamics are doing something qualitatively different from static optimization.
    \item \textbf{Ordering check:} The alignment analysis should continue to verify eigenvector ordering and sign conventions consistently across Hessian and covariance spectra before being used as a strong mechanistic signature.
\end{itemize}

\textbf{Recommended next pass:} Increase $d$, $n$, and training epochs for the key proof-of-concept experiments; widen the $\gamma$ sweep in the sharpness-ratio studies; add 5--10 seeds per configuration; report mean $\pm$ standard deviation or bootstrap confidence intervals; and add one equilibrium control for the linear model.

\subsubsection*{Next Research Plan: Scale-Up Toward Submission}

\textit{The current manuscript is proof-of-concept. The next pass should upgrade the experiment package toward submission-grade evidence for JMLR, Physical Review E, Neural Computation, or a main-conference submission to NeurIPS/ICML.}

\begin{enumerate}
    \item \textbf{Scale the core settings.} Re-run Exp.~1, Exp.~5, and Exp.~8 with $d=50$--$100$, $n_{\mathrm{samples}}=5000$--$10000$, and $n_{\mathrm{epochs}}=3000$--$5000$.

    \item \textbf{Widen the interpolation sweep.} Extend the $\gamma$ range to $10$ or even $20$ in Exp.~1, Exp.~3, and Exp.~8 so that the second-descent regime is fully resolved.

    \item \textbf{Replicate every key configuration.} Run at least 5--10 seeds per setting and report mean $\pm$ standard deviation or bootstrap confidence intervals for test loss, alignment ratios, sharpness ratio, erosion rate, and correlations.

    \item \textbf{Add an equilibrium baseline.} Include the minimum-norm pseudo-inverse solution for the linear model and compare it directly against SGD dynamics.

    \item \textbf{Strengthen curvature estimation.} Add convergence checks for Hessian power iteration and use the exact Hessian for the linear model as a validation benchmark for the approximate estimators.

    \item \textbf{Prioritize the decisive reruns.} Re-run Exp.~1 (curvature-noise coupling), Exp.~5 (erosion), and Exp.~8 (sharpness ratio) at the larger scale first; if these remain supportive, propagate the same protocol to Exp.~2, Exp.~3, Exp.~6, and Exp.~9.

    \item \textbf{Submission target.} If the larger-scale reruns preserve the current trends, the manuscript should be positioned for JMLR, Physical Review E, Neural Computation, or a NeurIPS/ICML main-track submission. If the trends weaken, narrow the claims to the regimes that remain robust.
\end{enumerate}

\textit{The objective of this pass is to convert proof-of-concept evidence into a statistically robust, submission-ready package.}
