...
\hline
Temperature $T$ & Thermal fluctuations & Effective temperature $T_{\text{eff}} = \eta/B$ \\
\hline
Thermal average $\langle \cdot \rangle$ & $\int (\cdot) P_{\text{eq}}(\sigma) d\sigma$ & Expectation over weight distribution \\
\hline
Disorder average $\overline{(\cdot)}$ & $\mathbb{E}_J[(\cdot)]$ & $\mathbb{E}_{\mathcal{D}}[(\cdot)]$ \\
\hline
Order parameter & Overlap $q = \frac{1}{N}\sum_i \sigma_i^{(1)} \sigma_i^{(2)}$ & Weight alignment between replicas \\
\hline
Phase transition & Paramagnetic $\leftrightarrow$ Spin glass & Under- $\leftrightarrow$ Over-parameterized \\
\hline
\end{tabular}
\end{center}
\end{definition}
