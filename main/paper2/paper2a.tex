...
% ... (Tiếp theo sau phần Double Descent trước đó) ...

% ============================================================================
% CHAPTER: FOUNDATIONAL DEFINITIONS FOR STATISTICAL PHYSICS OF LEARNING
% ============================================================================
% [Author's Note: This section addresses the fundamental requirement of GROUNDING
% all physical and mathematical concepts used in subsequent analysis. Each concept
% is defined formally, its components explained, and its physical/ML interpretation
% justified. This follows the principle: "Don't just describe, DEFINE and ARGUE."]
% ============================================================================

\section{Foundational Definitions: Grounding Physical Concepts in Learning Theory}
...
% ============================================================================
% AUTHOR'S NOTE: THEORETICAL REQUIREMENTS AND RESEARCH GAPS
% ============================================================================
% [The following subsection is intentionally placed at the beginning to provide
% transparency about what theoretical tools we require, what we have rigorously
% established, and where gaps remain. This follows the "Scientific Humility"
% principle: acknowledge what we don't know as clearly as what we do know.]
% ============================================================================

\subsection*{Theoretical Requirements and Current Gaps (Author's Note)}

\textit{Before proceeding, we explicitly enumerate the theoretical machinery required for this research program, along with an honest assessment of the current status of each component. This transparency is essential for maintaining intellectual integrity and guiding future work.}
...
\midrule
Statistical Learning Theory (SLT) & \textcolor{green!60!black}{Established} & Classical framework; reviewed in Section 2. VC-dimension, Rademacher complexity well-defined. \\
\addlinespace[2pt]
Bias-Variance Decomposition & \textcolor{green!60!black}{Established} & Valid for MSE loss and Bregman divergences; Section 3. \\
\addlinespace[2pt]
...
\begin{remark}[Research Philosophy]
The gaps identified above are not defects to be hidden but \textbf{research frontiers} to be explored. Each ``red'' item represents a potential contribution. The value of this framework lies not in having all answers, but in asking the right questions and providing a coherent language in which progress can be made.
\end{remark}

\begin{remark}[Methodological stance: scientific humility]\label{rem:scientific_humility}
Throughout this chapter, our goal is not to ``prove'' that deep learning \emph{is} a spin glass, but to construct a translation layer that is \emph{consistent with} what we can rigorously define on the learning side. When a step relies on an approximation (e.g., Langevin limits, equilibrium assumptions, replica calculations), we state it as such and treat the resulting predictions as \textit{testable hypotheses} rather than final verdicts. In this spirit, points of friction (missing assumptions, non-rigorous limits, empirical instabilities) are not merely defects to hide, but may indicate where a new theory is needed.
\end{remark}

% ============================================================================
% SECTION: THE MACHINE LEARNING SYSTEM - COMPLETE SPECIFICATION
% ============================================================================
% [Author's Note: This section responds to the requirement of specifying the
% "machine learning system" BEFORE introducing statistical physics language.
% We enumerate ALL components of the learning system in its native form,
% without any physics interpretation. This is the "honest starting point."
% Only after this complete specification do we introduce the physical analogies.]
% ============================================================================
...
% ============================================================================
% SECTION: RESEARCH FRICTION POINTS - DOCUMENTING VƯỚNG MẮC AS INSIGHT
% ============================================================================
% [Author's Philosophy: Following the principle that "vướng mắc là insight",
% we explicitly document the points of theoretical friction. These are not
% weaknesses to hide but research opportunities to embrace. Each friction
% point is a place where current theory breaks down and new theory is needed.]
% ============================================================================

\subsection{Research Friction Points: Where Theory Meets Its Limits}\label{sec:friction_points}

\textit{In the spirit of scientific humility, we document the ``friction points'' where our theoretical machinery encounters resistance. These are not failures but \textbf{signposts} indicating where deeper understanding is needed.}

\subsubsection{Friction Point 1: The Langevin Approximation Breaks Down}

\textbf{The Problem:} The continuous-time Langevin equation (Definition~\ref{def:langevin_sgd}) is derived under the assumption that the learning rate $\eta$ is small. However, in practice:
\begin{itemize}
    \item Large learning rates are often used, especially in early training.
    \item Learning rate schedules (warmup, decay) violate the constant-$\eta$ assumption.
    \item The discretization error $O(\eta^2)$ may accumulate over millions of steps.
\end{itemize}

\textbf{The Insight:} The breakdown of Langevin suggests that \textit{discrete-time effects} may be essential to understanding SGD. Recent work on ``edge of stability'' training \cite{cohen2021gradient} shows that large learning rates induce qualitatively different dynamics (oscillation, catapulting) not captured by continuous diffusion.

\textbf{Open Question:} Can we develop a \textbf{discrete-time statistical mechanics} that captures these effects? What is the ``Hamiltonian'' for discrete SGD?

\subsubsection{Friction Point 2: The Replica Trick's Mathematical Fragility}

\textbf{The Problem:} The replica identity (Eq.~\eqref{eq:replica_identity}) involves:
\begin{enumerate}
    \item Computing $\mathbb{E}[Z^n]$ for integer $n$.
    \item Analytically continuing to real $n$.
    \item Taking the limit $n \to 0$.
\end{enumerate}

Each step is mathematically problematic:
\begin{itemize}
    \item \textbf{Analytic continuation:} There are infinitely many functions that agree on integers but differ on the reals. Which continuation is ``correct''?
    \item \textbf{The $n \to 0$ limit:} What does it mean to have ``zero replicas''? The physical interpretation is obscure.
    \item \textbf{Replica symmetry breaking (RSB):} In some systems (e.g., Sherrington-Kirkpatrick spin glass), the ``obvious'' symmetric ansatz gives wrong results. One must postulate a hierarchy of symmetry breaking whose physical origin is debated.
\end{itemize}

\textbf{The Insight:} The replica trick \textit{works empirically} despite lacking rigorous justification. This suggests either:
\begin{enumerate}
    \item There exists a deeper mathematical structure we have not yet identified.
    \item The agreement with experiment is a ``lucky accident'' that may fail in edge cases.
\end{enumerate}

\textbf{Research Direction:} Alternative methods (cavity method, interpolation techniques, supersymmetric approach) can sometimes verify replica results rigorously. A systematic comparison in the ML context is needed.

\subsubsection{Friction Point 3: The Temperature Analogy's Limits}

\textbf{The Problem:} We defined $T_{\text{eff}} = \eta/B$ as the ``effective temperature'' (Definition~\ref{def:effective_temp}). But this analogy has limits:
\begin{itemize}
    \item \textbf{No detailed balance:} In equilibrium thermodynamics, $T$ determines the Boltzmann distribution via detailed balance. SGD generally violates detailed balance.
    \item \textbf{Direction-dependent temperature:} The noise covariance $\Sigma(W)$ is a matrix, not a scalar. Different directions in weight space experience different ``temperatures.''
    \item \textbf{Temperature depends on position:} $\Sigma(W)$ varies across weight space, so the ``temperature'' is a field, not a constant.
\end{itemize}

\textbf{The Insight:} The scalar temperature $T_{\text{eff}}$ is a \textbf{zeroth-order approximation}. The full physics requires treating the diffusion tensor $D(W) = (\eta/B) \Sigma(W)$ as the fundamental object. This is precisely what our NESP framework attempts.

\textbf{Open Question:} Is there a well-defined ``temperature field'' $T(W)$ that captures the local noise intensity? If so, how does it relate to generalization?

\subsubsection{Friction Point 4: The Gap Between Linear and Nonlinear}

\textbf{The Problem:} Our toy model (Section~\ref{sec:toy_model}) is linear. Real neural networks use nonlinear activations (ReLU, sigmoid, etc.). The gap is substantial:
\begin{itemize}
    \item \textbf{Hessian structure:} In linear models, the Hessian is constant or slowly varying. In ReLU networks, the Hessian is \textit{piecewise constant} with discontinuities at activation boundaries.
    \item \textbf{Loss landscape topology:} Linear models have a single connected basin (or manifold) of minima. Nonlinear models may have exponentially many disconnected basins.
    \item \textbf{Feature learning:} Linear models cannot learn features---the ``representation'' is fixed. Nonlinear networks fundamentally change what they compute during training.
\end{itemize}

\textbf{The Insight:} The linear model is the \textbf{hydrogen atom} of learning theory---solvable but not representative. The challenge is to identify which properties (e.g., curvature-noise coupling) are \textit{universal} versus \textit{model-specific}.

\textbf{Research Direction:} A systematic ``ladder of models'' from linear $\to$ random features $\to$ shallow ReLU $\to$ deep ReLU would reveal which phenomena persist and which disappear.

\subsubsection{Friction Point 5: Observational vs.\ Causal Claims}

\textbf{The Problem:} Much of double descent research is \textit{observational}: ``we see a peak at $P \approx N$.'' Our NESP framework attempts \textit{causal} claims: ``the peak occurs \textit{because} of curvature-noise coupling.'' But establishing causality is difficult:
\begin{itemize}
    \item \textbf{Confounders:} Many things change at $P \approx N$ (condition number, number of minima, basin geometry). Which is the ``cause'' of the peak?
    \item \textbf{Interventions:} A true causal claim predicts what happens under \textit{intervention}. If we artificially modify the noise structure, does double descent disappear as predicted?
\end{itemize}

\textbf{The Insight:} Our framework makes \textbf{falsifiable predictions} (Section~\ref{sec:experimental_agenda}). If these predictions fail, the theory must be revised. This is the scientific method at work.

\textbf{Concrete Test:} Train with \textit{isotropic} noise (add Gaussian noise independent of gradient). If curvature-noise coupling is essential, double descent should be suppressed or qualitatively different.

\begin{remark}[The Value of Friction]
These friction points are not embarrassments to be minimized in the final write-up. They are the \textbf{growing edge} of the theory---the places where new insights will emerge. A theory without friction points is either trivial or dishonest.
\end{remark}
...
% ============================================================================
% EXPANDED: REPLICA METHOD DERIVATION AND LIMITATIONS
% ============================================================================
% [Author's Note: Following the request to provide more detail on the Replica
% Method, we now expand this section with: (1) explicit derivation steps,
% (2) physical interpretation at each stage, (3) honest acknowledgment of
% mathematical gaps. This is the "completeness of details in a sensitive
% setting" that the research collaboration requires.]
% ============================================================================

\subsubsection{The Replica Calculation: Step-by-Step (Expanded)}

\textit{We now provide a more detailed exposition of the replica calculation, highlighting both its power and its fragility. This level of detail is necessary to understand both what the method achieves and where it may fail.}
...
% ============================================================================
% CONCEPTUAL BRIDGES: CONNECTING FRAGMENTED IDEAS
% ============================================================================
% [Author's Note: This section addresses the concern that "everything will fail
% and misrepresentation will happen" if we don't explicitly connect the pieces.
% Each bridge represents a logical connection that might otherwise be hand-waved.]
% ============================================================================

\subsection{Conceptual Bridges: Making Implicit Connections Explicit}\label{sec:conceptual_bridges}

\textit{Before proceeding to the NESP framework, we pause to make explicit several conceptual bridges that might otherwise be ``hand-waved.'' Each bridge connects two ideas that are often assumed to be related but whose connection requires justification.}
...
\section{A Non-Equilibrium Statistical Physics Framework}

To bridge the gap between static theory and dynamic practice, we propose a synthesis of \textbf{Multi-scale Training Dynamics} and \textbf{Critical Landscape Geometry} (Jamming). This framework treats SGD not merely as an optimizer, but as a driven physical system interacting with a disordered potential landscape.

\subsection{Modeling SGD Dynamics: The Langevin Approximation}
...
\begin{equation}
    dW_t = -\nabla \mathcal{L}(W_t) dt + \sqrt{2D(W_t)} dB_t
\end{equation}
where $B_t$ is Brownian motion. Crucially, the diffusion matrix $D(W_t)$ arising from SGD noise is \textit{anisotropic} and \textit{state-dependent}. Unlike uniform thermal noise in classical thermodynamics, SGD noise magnitude scales with the gradient covariance. Recent studies \cite{chaudhari2018stochastic} suggest that this specific noise structure introduces an \textit{implicit regularization} term that penalizes ``sharp'' minima, effectively driving the system toward ``flat'' regions of the landscape which correlate with better generalization.
...
\subsection{The Jamming Transition Landscape}
The geometric ``stage'' upon which this dynamics plays out is described by the \textbf{Jamming Transition} analogy \cite{geiger2019jamming}. The interpolation threshold corresponds to the transition point where the system goes from an under-constrained (liquid/floppy) phase to an over-constrained (jammed/solid) phase.

\begin{itemize}
    \item \textbf{Under-parameterized ($P < N$):} The system is jammed; zero training error is inaccessible due to excessive constraints.
    \item \textbf{Critical ($P \approx N$):} The system is marginally stable. The landscape is extremely rugged with sharp, narrow minima, analogous to a jamming point in granular materials.
    \item \textbf{Over-parameterized ($P > N$):} The constraints are relaxed (unjamming). The landscape opens up new dimensions, creating a network of connected, flat global minima.
\end{itemize}
...
% ============================================================================
% CHAPTER 7: NON-EQUILIBRIUM STATISTICAL PHYSICS FRAMEWORK
% ============================================================================
% [Author's Note: This chapter represents the theoretical core of our NESP approach.
% Unlike the previous sections which reviewed existing literature, here we develop
% original theoretical machinery. The structure follows a physics-style derivation:
% (1) Identify the failure of existing assumptions, (2) Propose new postulates,
% (3) Derive consequences, (4) Design verification protocols.
% This is ACTIVE RESEARCH - no definitive conclusions are drawn.]
% ============================================================================
...
% [Author's Reflection: This motivation section is deliberately personal. In scientific
% writing, we often hide our intellectual journey behind a facade of logical inevitability.
% But research is not like that. We make choices---often based on aesthetic preferences,
% analogies from other fields, or sheer stubbornness. Acknowledging this does not weaken
% the science; it makes it more honest.]
...
% [Author's Reflection: This postulate is the conceptual foundation of our approach.
% It shifts the theoretical question from "what is the equilibrium?" to "how does
% the system dynamically select among degenerate equilibria?" This is analogous to
% symmetry breaking in phase transitions, but driven by noise structure rather than
...
The SGD noise covariance $\Sigma(W)$ is not isotropic white noise. Its structure is intrinsically coupled to the local curvature of the loss landscape, approximated by the Hessian $H(W) = \nabla^2 \mathcal{L}(W)$:
\begin{equation}\label{eq:noise_hessian_coupling}
    \Sigma(W) \approx \alpha \, H(W) + \mathcal{O}(H^2)
\end{equation}
for some coupling constant $\alpha > 0$ that depends on the data distribution and batch statistics.
\end{hypothesis}

% [Technical Note: The precise form of Eq.~\eqref{eq:noise_hessian_coupling} requires
% careful derivation. For MSE loss with batch sampling, the gradient covariance involves
% terms like $\mathbb{E}[(x x^T) \otimes (x x^T)]$. The connection to the Hessian emerges
% in certain limits (e.g., when data covariance is close to identity). A rigorous proof
% for general architectures remains an OPEN PROBLEM.]
...
\subsection{Entropic Selection: The ``Survival of the Flattest''}

Building on Hypothesis~\ref{hyp:curvature_noise}, we now articulate the selection mechanism that distinguishes among degenerate minima.
...
In equilibrium systems (Kramers theory), $\tau \sim \exp(\Delta E / k_B T)$ depends on the energy barrier $\Delta E$ and uniform temperature $T$. However, under NESP with curvature-dependent noise, the escape time depends on the \textit{local landscape geometry}:

\begin{equation}\label{eq:escape_time_nesp}
    \tau(W) \sim \exp\left( - \beta_{\text{eff}}(W) \cdot \lambda_{\max}(W) \right)
\end{equation}
where $\lambda_{\max}$ is the maximum Hessian eigenvalue and $\beta_{\text{eff}}$ is an effective inverse temperature that itself depends on the noise structure.

% [Author's Reflection: Eq.~\eqref{eq:escape_time_nesp} is currently a qualitative
% scaling argument, not a rigorous derivation. The precise functional form requires
% solving the Fokker-Planck equation with state-dependent diffusion, which is
% analytically intractable for general landscapes. This is a KEY OPEN PROBLEM
% that we address via the toy model in Section~\ref{sec:toy_model}.]
...
This interpretation connects directly to the \textbf{Effective Model Complexity} (EMC) introduced by \cite{nakkiran_deep_2019} and reviewed in Section 4. In their framework, EMC is defined operationally as the maximum number of samples a model can interpolate. Our NESP framework provides a \textit{physical mechanism} for why EMC matters: at the EMC threshold, the landscape geometry undergoes a jamming transition, and the noise-curvature coupling reaches its maximum. The EMC is not merely a descriptive statistic; it is the \textbf{order parameter} of a dynamical phase transition.
\end{remark}

% ============================================================================
% SECTION 7.4: TOY MODEL FOR ANALYTICAL VERIFICATION
% ============================================================================
% [Author's Note: This section is crucial for the scientific credibility of the
% NESP framework. We need a tractable system where the Hessian and noise covariance
% can be computed explicitly. The two-layer linear network is ideal: it has non-convex
% loss (quartic in parameters) but admits closed-form analysis.]
%
% [Execution Philosophy: We now move from qualitative arguments to EXPLICIT 
% MATHEMATICAL DERIVATION. Every equation must be traceable. The goal is to
% verify Hypothesis 7.2.1 (Curvature-Noise Coupling) analytically, not just
% argue for it on physical intuition.]
% ============================================================================
...
% [Author's Reasoning: Why not use a deep nonlinear net? Because we CANNOT compute
% the Hessian analytically. The linear model is the simplest system that still
% exhibits (1) non-convex loss, (2) degenerate minima, (3) double descent behavior.
% It is the "hydrogen atom" of learning theory.]
...
% [Technical Note: Fixing $v$ is a standard simplification in the literature
% (see Saxe et al., 2014). It removes one layer of non-convexity while preserving
% the essential quartic structure in $U$. The full $(U, v)$ case can be treated
% similarly but with more bookkeeping.]
...
% [Checkpoint: The population loss is simply half the squared distance between
% the effective weight and the teacher. The global minimum is achieved when
% $U^\top v = w^*$. But there are INFINITELY MANY such $U$ matrices! This
% degeneracy is the key to understanding the non-equilibrium selection.]
...
% [Author's Reflection: This result is STRUCTURALLY CRUCIAL. The gradient is a
% rank-1 outer product of the readout weights $v$ and the input $x$, scaled by
% the current prediction error $e(x)$. This rank-1 structure will reappear in
% both the Hessian and the noise covariance, creating the coupling we seek.]
...
% [Technical Note: The tensor product here is between $k \times k$ and $d \times d$
% matrices, yielding a $(kd) \times (kd)$ covariance matrix when $U$ is vectorized.
% The structure $(v v^\top) \otimes (x x^\top)$ is KEY---it factorizes into the
% "hidden unit" structure and the "input space" structure.]
...
% [Author's Note: This exact formula can be verified by direct computation or
% by using the general formula for Gaussian fourth moments. The important point
% is that $\Sigma$ depends on the SAME quantities as the Hessian will.]
...
% [CRITICAL OBSERVATION: The population Hessian is INDEPENDENT of the current
% weights U! This is a special property of the linear model. The curvature is
% constant across the entire parameter space. However, the NOISE (Eq. 7.sigma)
% DOES depend on U through the error term e(x).]
...
% [Author's Reflection: This is the key analytical confirmation we were seeking. The
% structural identity between H and Sigma is not an accident---it arises from
% the fact that both are constructed from the same gradient outer product.
% The question now is: does this survive in nonlinear networks?]
...
% [Author's Note: The mathematical divergence at $\gamma = 1$ is the RMT signature
% of the interpolation threshold. In our dynamical framework, this translates to
% a "stochastic resonance" where noise and curvature conspire to destabilize the
% system maximally. This is WHY the test error peaks at the threshold.]
...
% [Author's Reflection: The phase structure maps cleanly onto the double descent
% curve. The key insight is that Phase III is NOT simply "having more parameters."
% It is a qualitatively different dynamical regime where the NOISE STRUCTURE
% changes from destabilizing (at criticality) to guiding (in the flat manifold).]
...
% [Author's Note for Future Work: The numerical experiments to verify these
% predictions should be straightforward to implement. The key is to compute
% the Hessian eigenvalues during training, not just at convergence. This will
% reveal the DYNAMICAL nature of the phase transition.]

% ============================================================================
% SECTION 7.4.8: CRITICAL SELF-ASSESSMENT - LIMITATIONS OF THE TOY MODEL
% ============================================================================
% [Author's Philosophy: A good theoretical physicist knows the BOUNDARIES of
% their model as well as its predictions. This section is essential for
% intellectual honesty and for guiding future research directions.]
% ============================================================================
...
\begin{quote}
\textbf{Author's Conjecture (Unverified):} The coupling likely persists \textit{qualitatively} but not \textit{quantitatively}. The physical intuition is robust: sharp directions should still correlate with high gradient variance because both are driven by the same data statistics. However, the precise proportionality constant $\alpha$ in $\Sigma \approx \alpha H$ may become a complicated function of the activation pattern.
\end{quote}
...
% ============================================================================
% SECTION 7.5: OPEN PROBLEMS AND RESEARCH DIRECTIONS (EXPANDED)
% ============================================================================
...
% ============================================================================
% SECTION 7.6: EXPERIMENTAL AGENDA
% ============================================================================
...
% ============================================================================
% AUTHOR'S NOTES FOR FUTURE DEVELOPMENT
% ============================================================================

\subsection*{Author's Notes: Research Trajectory and Pickup Points}
...
% ============================================================================
% SECTION 7.7: PHILOSOPHICAL REFLECTIONS AND EPISTEMOLOGICAL SHIFT
% ============================================================================

\subsection{Philosophical Reflections: The Epistemological Shift}
...
% ============================================================================
% SECTION: COMPREHENSIVE VƯỚNG MẮC LOG - RESEARCH FRICTION AS INSIGHT
% ============================================================================
% [Author's Philosophy: Following the principle that "vướng mắc hay bất cứ cái
% vấn đề gì gặp phải đều là 1 insight dùng trong theory construction với analysis",
% we now provide a comprehensive log of all encountered obstacles, their current
% status, and how each might lead to theoretical insights.]
% ============================================================================

\subsection{Comprehensive Research Friction Log}\label{sec:friction_log}

\textit{``Vướng mắc là insight.'' This section documents every significant obstacle encountered during the development of this framework, treating each as a potential source of theoretical insight rather than a mere defect to be hidden.}
...
\begin{description}
    ...
    \textbf{Vướng mắc:} The analytic continuation from integer $n$ to real $n \to 0$ is not unique. Different continuations give different results. \\
    ...
    \textbf{Vướng mắc:} When $D(W)$ depends on $W$, the stationary distribution $P_{\text{stat}}(W)$ is not the simple Boltzmann form. Closed-form solutions exist only for 1D or special cases. \\
    ...
    \textbf{Vướng mắc:} Standard SDE theory requires $\eta \to 0$. In practice, $\eta$ is finite and sometimes large. The error accumulation over training is not controlled. \\
...
\begin{description}
    ...
    \textbf{Vướng mắc:} Physical temperature is a scalar; SGD noise covariance is a matrix. The mapping $T_{\text{eff}} = \eta/B$ loses information about anisotropy. \\
...
% ============================================================================
% SECTION: RESEARCH ROADMAP - CONCRETE ACTION PLAN
% ============================================================================
...
\subsection*{Closing Reflection: The Invitation}

\textit{``The work in this chapter is not a conclusion; it is an invitation.''}

We have constructed a framework---NESP---that reframes double descent as a dynamical phase transition driven by the anisotropic noise structure of SGD. We have identified the key hypothesis (curvature-noise coupling), derived analytical predictions in a toy model, and designed experiments to test them. We have documented our uncertainties (friction points) and our limitations honestly.

What remains is the hard work of verification. The theory makes falsifiable predictions. If they hold, we will have made progress toward understanding why deep learning works. If they fail, we will have learned something equally valuable: that the true mechanism lies elsewhere.

Either way, the scientific process advances. This is the nature of research---not a march toward predetermined conclusions, but a dialogue with nature, mediated by mathematics and experiment.

\textit{The work in this chapter is not a conclusion; it is an invitation.}

% ============================================================================
% AUTHOR'S CRITICAL SELF-REVIEW: COMPREHENSIVE VƯỚNG MẮC ANALYSIS
% ============================================================================
% [This section documents the results of a systematic review conducted on
% February 1, 2026, identifying critical gaps, hand-waving, and overclaiming
% that must be addressed before the paper can be considered complete.]
% ============================================================================

\subsection*{Author's Critical Self-Review (February 2026)}
