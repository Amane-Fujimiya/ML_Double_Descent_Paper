...
\begin{equation}
    \tau = \mathcal{M}(\mathcal{H}), \quad \tau = \gamma 
\end{equation}
hence the escape trajectory is then controlled subsequently. The parameter $\tau$ then can be computed, accordingly for the optimization term as 
\begin{equation}
...
\end{equation}
for arbitrary constant $I,\alpha,p$, of which now the addition operator would be slightly adjust, such that the sum will contain only the LHS of the operator if the RHS is in its undefined domain, and otherwise they add on together. 
...
Regarding double descent, \cite{nakkiran2019moredata} explored the problem, in which the increment of sample actually hurts linear regression process. 
...
\section{Reformation of learning theory}
