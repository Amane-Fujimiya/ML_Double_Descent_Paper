...
\begin{figure}[htb]
    ...
    \caption{Prediction made by certain configurations of Hypothesis~\ref{hyp:interpolation_tau}. Here, the injected PAC-type decay term provides the global envelope, while the interpolation offset term introduces singular behaviour near capacity-sample equality. The resulting superposition reproduces a family of double-descent-like curves depending on the trajectory in $(m,|\mathcal{H}|)$-space.}
\end{figure}
...
\end{enumerate}
Inherently, $\tau$ is also empirical, and thus $q(\tau)$. Such expression in general appears in KL-convergence of PAC-Bayes, and linear models isotropic design gives $\sigma^{2}p/(m-p-1)$, which is fairly similar to what we have. Nevertheless, one should realize that current theories are not equipped to express this in particularly detailed insight. In certain cases, it only serves as heuristics or empirical presumptions, or rather theoretical speculation in a non-constructive way. Imitating double descent aside, this prompts us to seriously tackle the double descent problem, or otherwise the messiness of analysis will continue. For a brief explanation on why this is true, we can agree on an exercise. Despite appearing different and using different control nobs, the major classical theories all rely on the same structural primitives, where the dataset is assumed to be sampled from independent $\mathcal{D}$-distributing sampling operator for $m$ samples of dataset $\mathcal{S}$, hypothesis class $\mathcal{H}$ arbitrarily, risk $R(h)$, and the empirical risk $\hat{R}(h)$, or $\hat{R}_{\mathcal{S}}(h)$ on strictly $\mathcal{S}$-conditioned evaluation space. The theory only attempts to control $R(h)-\hat{R}_{\mathcal{S}}(h)$ or its variants in the general risk expression, using a capacity measure. This can be for (1) VC theory, as VC dimension; (2) Rademacher theory as the Rademacher complexity; (3) Occam learning and compression scheme using description length, and PAC-Bayes for KL-divergence between posterior and prior. Those are different regularizers on the same skeleton, but they also inherit the same structural blind spot of PAC. More specifically, there exists no variables in-theory that describe interpolation rank or the existence thereof, feature geometry, optimisation trajectory, and formal condition on parameterization structure and organization. Usually, the bounds are always
\begin{equation}
    R(h) \leq \hat{R}_{\mathcal{S}}(h) + \mathcal{O}(|\mathcal{H}|), \quad \mathcal{O}(|\mathcal{H}|) \in \left\{\sqrt{\frac{\mathrm{comp}(h)}{m}}, \frac{\mathrm{KL}(Q||P)}{m}, \mathfrak{R}_{m}(\mathcal{H})\right\}
\end{equation}
of which presuppose complexity condition. Even NTK as a modern analogue suffers the same problem, but with different reason than the PAC-family of derivatives. For a quick review, we see that consider a neural network $f(x;\theta)$ on parameterized complexity scheme $\theta\in\mathbb{R}^{P}$ of parameter space, Neural Tangent Kernel (NTK) is defined as
\begin{equation}
    \begin{split}
        \Theta(x,x',\theta) = \nabla_{\theta}f(x;\theta) \cdot \nabla_{\theta}f(x';\theta) 
         \equiv \sum^{P}_{p=1} \frac{\partial f(x;\theta)}{\partial \theta_{p}} \frac{\partial f(x';\theta)}{\partial \theta_{p}}
    \end{split}
\end{equation}
for $x'$ as another input point in the input space, thus NTK is a local comparison scheme of the kernel to show how the network output changes with respect to its parameter. NTK can be said to determine the training dynamics in function space, and its main result, where in \cite{Jacot:2018:NTK} is stated as
\begin{equation}
    \Theta(x,x'\theta(t)) \to \Theta_{\infty}(x,x'), \quad n_{\ell}\to \infty
\end{equation}
where $_{\ell}$ denotes the layer widths, and the result takes that it remains constant during training. This collapses the network into linearized initialization condition, for training becomes kernel version of gradient descent, and furthermore, prediction is essentially collapsed to kernel regression with the kernel $\Theta_{\infty}$. Such is to say, in the NTK regime, parameters move infinitesimally, for the kernel to remain \textbf{fixed}, and static feature space, the model becomes kernel regression in fixed feature space. Structural consequence is that it freezes the feature representation, the learning problem is reduced to the condition on 
\begin{equation}
    \mathsf{ERM}_{\mathrm{NTK}}(h) \equiv \min_{\alpha\in\mathbb{R}^{m}} \hat{R}(\alpha) \equiv \min_{\alpha\in\mathbb{R}^{m}} \sum^{m}_{i=1} \left(\sum^{m}_{j=1}\alpha_{j}\Theta(x_{i},x_{j})-y_{i}\right)^{2} 
\end{equation}
for empirical risk minimization, with closed form solution as for $\alpha$ learnable coefficients being $(K+\lambda I)^{-1}y$, which is essentially kernel ridge regression. The behaviours are now flat into eigenvalue of $K$, the sample size $m$, and the noise level. Because of such reduction, NTK struggles with phenomena, either by structural incompatibility or bloats, in explaining representation learning, hierarchical feature formation, deep network scaling laws in loose form, neural collapse phenomenon, large model generalization behaviours, and specifically relevant as \textbf{double descent empirical regimes}. Thus, the reason is the same absence of variables, but the reason differs within simplifications. 

That said, what do we gain from this modification? Specifically, we have shown that the coupling on the term, loosely said as \textbf{interpolation threshold} for the lack of better word, though much of the opposite, can be used to inject structures into the conditioned space that PAC bound still works, but cannot or rather wasn't equipped of its analytical toolkit to analyse, but attains double descent. Even in PAC-setting, the effective counteraction would only to recognize of which traversing paths are there of the $(m,|\mathcal{H}|)$-space we are operating on, and the smooth convergence curve of PAC-setting is also consistent with the explanation, as it restricts into a landscape, but not remove the probability of extension on itself. By then, we can identify three fixes required of a deeper theoretical analysis: the hypothesis growth trajectory $q(\tau)$, the sample growth $p(\tau)$, and the interpolation offset, all of which can be dependent on various properties of extended system in hindsight, and of which are dependent on various transitional assumptions that cannot be structurally derived on PAC, even if it allows for such. This is extended also to VC-theory and a class of assumption-based, Rademacher-similar heuristics on model complexities. Lastly, we emphasize from this observation that, (1) geometry is not inherently only in $|\mathcal{H}|$, especially if we proceed to more dynamically complex and often pathological work cases; (2) worse-case bounds are loose enough for us to inject geometry into it, though it means conditions are still possibly consistent with the later analysis; (3) deep networks are inherently different, however, an action of streamlining model interpretability and structural functions of such, different from simply mathematical reduction is necessary. An example of the last point is explained in the later chapters, where we investigate the polynomial system, where of which it is actually that we are using a very poorly conditioned Vandermonde polynomial basis, much less of the simple mathematics on model descriptions. 
    ...
    \begin{equation*}
        \begin{split}
            ...
            &= \lambda_{1}\underbrace{\left\{ \mathbb{E}_{\mathcal{D}}[f(x;\mathcal{D})] , \mathbb{E}[y\mid x] \right\}}_{\text{bias term}} 
             +\lambda_{2} \underbrace{\mathbb{E}_{\mathcal{D}} \left\{(f(x;\mathcal{D}), \mathbb{E}_{\mathcal{D}}[f(x;\mathcal{D})])\right\}}_{\text{variance term}} 
            +\underbrace{\lambda_{3}\epsilon}_{\text{irreducible error}}
        \end{split}
        \end{equation*}
...
The question against double descent is still rather elusive. Ever since the discovery in \cite{belkin_reconciling_2019}, despite the general chaotic view of double descent that seems to hold up pretty well, many researches have been conducted throughout, trying to observe or explain it, as we briefly mentioned \cite{nakkiran_deep_2019} as one of such main paper. Before attempting to do anything, and to further contribute with hypotheses forward, we will have to resolve to review previous coursework on such matter. Most of those works include some on theoretical analysis, some on dissection of optimization method then speculation of possible hindsight, some on experiments and observable effects, and so on. Furthermore, sections specifically designed to host experiments and observations will also offer a clearer insight into how this can be done. The task is then for us to review those papers, to grip a sense on what is actually operating inside the domain of analysis. Theoretical insights are often of less emphasis, as double descent occurs at the mainly underdeveloped area of deep and machine learning, and as also for theoretical attacks to simplify their system for convergences bound, leaving mechanisms behind in determining such structures. 

The main goal of attacking from theoretical side, of double descent stemmed from the observation in which there exists irregularity in the model expression behaviours. As such, the first thing to do is to contrive the consensus on `what' constitutes double descent, at least per observable results, by delivering a conjecture. We deliver the phenomenological conjecture as below. 

\begin{conjecture}[Probing double descent]
...
With much questions going on, we are ready to follow up the literatures in provision of said questions. Most of them are in consideration of all the questions above, but under different strategies and either phenomenological, or deliberately simplified theoretical studies. Let us also recite what happens within the setting of many double descent results. 
...
\end{equation}
where $\bm{z}\mathbb{R}^{d}$ is the teacher's input and $y^{*},y\in\mathbb{R}$ are the teacher's noiseless and noisy outputs, respectively. Thus, the setting itself is rather familiar, within which the structure of the oracle is assumed and embedded of the observation space clarified noise $\epsilon$. In a sense, this is a case of \textbf{external fixture} of missing information, such that to posit no information package is not present, but most are noise intrinsic of the setting or retrieval purpose. $\mathbf{W}\in\mathbb{R}^{d}$ represents the weights, assumed as \textbf{fixed} in the setting of the teacher and $\epsilon\in\mathbb{R}$ is the label noise. Here, both $W_{i}$ and $\epsilon$ are assumed to be drawn i.i.d. from Gaussian distributions with zero means and variances of $1$ and $\sigma^{2}$, respectively. Within this assumption, though, it means practically nothing as the noise is treated simply as intrinsic and not structural. Admission of such simply gives i.i.d. and Gaussian form a kind of statistical expectation, nothing more, of which can be reasonably extrapolated from the hypothesis class statistics itself. Despite this structural shallowness, the result still has to be analysed to be further criticized, especially considering it is of about multiscale feature learning. 


\subsection{Reinforcement learning and double descent - \cite{brellmann2024on}}

In this paper, the author themselves, contributed to the analysis of double descent inside reinforcement learning, specifically LSTD - Least-squares (LS) for the class of \textbf{temporal difference} (TD)\index{temporal difference} algorithms. Interestingly, one of the main contribution of the paper, is the focus on the \textit{double asymptotic regime} for a new way of defining \textbf{model complexity}, in specificity. The analysis is strictly heuristic and simple, as seen of linear model and nonlinear random features for approximation of overparameterized single-hidden-layer network in the lazy training regime, for that we do not actually quite sure how it is overparameterized in what way. However, one of the fairly suspicious component, lies in the asymptotic regime itself. Fortunately, the authors working on the paper provided a rather meticulous set of assumptions, in which we will make use for the clear picture of what is in the framework itself, though under full dissection, we have a rather large system for criticism underlying such. First, let us set up the scenario. LSTD is part of a class of solution, for Markov reward processes, essentially a rather complex under mathematical representation, but simple technically speaking transition reward learning optimization scheme, basing itself on the global observed state space restriction. Of such, LDA, or \textbf{Linear Function Approximation}, is the family of techniques LSTD is based on, because of the practical infeasibility to solve
\begin{equation}
   \bm{V}=\bar{\bm{r}} + \gamma \bm{P} \bm{V},\quad \mathrm{MSBE}(\theta)
=
\left\|
\bar{\bm{r}}
+
\gamma \bm{P} \Sigma_\mathcal{S}^{\top}\bm{\theta}
-
\Sigma_\mathcal{S}^{\top}\bm{\theta}
\right\|_{\bm{D}_{\bm{\pi}}}^{2},
   \label{def:bellman_eq_MSE}
\end{equation}
for it being the unique fixed-point Bellman's equation, where $\bm{V} \in \mathbb{R}^{\lvert \mathcal{S} \rvert}$ is a vector whose $i$-th element is the value function of the $i$-th state $\mathcal{S}_{i}$; and $\bar{\bm{r}} \in \mathbb{R}^{\lvert \mathcal{S} \rvert}$ is the vector containing the expected rewards, for which $\bar{\bm{r}}_{i} = \mathbb{E}_{\bm{P}}[r|\bm{S}_i]$ for all $i \in [~\lvert \mathcal{S} \rvert~]$. The setting itself is deliberate, when fixing the LSTD estimator in the paper, the analysis reduces to reinterpretation on \textbf{lazy training and random features} analysis, thus the TD operator becomes 
\begin{equation}
    \mathbf{A} = \Sigma_{\mathcal{S}} \mathbf{D}_{\bm{\pi}} \left(\mathbf{I}_{|\mathcal{S}|} - \gamma \mathbf{P}\right) \Sigma_{\mathcal{S}}^{\top}
\end{equation}
which equals to random matrix with structured covariance case. The fixture of the LSTD in the paper also reduces the observations made in the paper into $N/m\approx 1$ on the matrix singularity transition behaviours, not much different than linear regression double-descent interpretation we analysed earlier. 
\begin{assumption}[Double Asymptotic Regime,\cite{brellmann2024on}]
    As $N, m, d \to \infty$, we have:
    \begin{enumerate}[itemsep=1pt,topsep=1pt]
        \item $0 <  \lim\min \left\{\tfrac{N}{m}, \tfrac{d}{m}, \tfrac{m}{\lvert \mathcal{S} \rvert}\right\} < \lim \max \left\{\tfrac{N}{m}, \tfrac{d}{m}, \tfrac{m}{\lvert \mathcal{S} \rvert}\right\} < \infty$. 
        \item There exists $K_\mathcal{S}, K_r>0$ such that $\lim \sup_{\lvert \mathcal{S} \rvert} \lVert \mS \rVert < K_\mathcal{S}$ and $r(\cdot, \cdot)$ is bounded by $K_r$. 
    \end{enumerate}
    \label{assumption:growth_rate}
\end{assumption}

\begin{assumption}[Bounded Eigenspectrum, \cite{brellmann2024on}]
    There exist $0 < \xi_{\min} < \xi_{\max}$ such that for every $m$, all the eigenvalues of $H(\hat \mA_m)$ are in $[\xi_{\min}, \xi_{\max}]$.
    \label{assumption:regime_n}
\end{assumption}

\begin{assumption}[Run-off growth, \cite{brellmann2024on}]
    As $n,m \to \infty$, we have 
    \begin{equation}
        \left\lVert \mathbf{A}_n - \mathbf{D}_{\bm{\pi}}\left[\mathbf{I}_{|S|} - \gamma \mathbf{P}\right] \right\rVert= \mathcal{O}\!\left(\frac{1}{\sqrt{n}}\right)
    \end{equation}
    \label{assumption:new_regime_n}
\end{assumption}
\begin{figure*}[t] 
    \begin{center} 
    \subfloat[Taxi-v3 \label{fig:taxi_gymnasium}]
    {
        \includegraphics[width=0.25\linewidth]{img/taxi.png}
    } \hspace{2cm}
        \subfloat[MSBE as a function of $N/m$]
        {
            \input{pdf/msbe_err.tex}
            % \includegraphics[trim=60 80 60 80, width=0.5\linewidth]{img/taxi/msbe.png}
        } 
        \caption{\textbf{As the model complexity $N/m$ (for $N$ parameters, $m$ distinct visited states) increases, the MSBE first shows a U-shaped curve, peaking around the interpolation threshold ($N=m$). Double descent refers to the phenomenon for $N/m>1$ where the MSBE drops once again.} Continuous lines (\textcolor{red}{red}) indicate the theoretical values, the crosses (\textcolor{Blue}{blue}) are numerical results averaged over $30$ instances with their standard deviations after the learning with regularized LSTD on Taxi-v3 for $\gamma=0.95, \lambda=10^{-9}, n=5000, m=310$. Taken from \cite{brellmann2024on}}
        \label{fig:double_descent_taxi_intro}
    \end{center}
\end{figure*}
Those assumptions, while looking rather innocent, are rather not trivial from what is assumed. For Assumption~\ref{assumption:growth_rate}, the first sub-assumption is structurally the family of assumptions used in random matrix theory, and extensively utilized under context of modern high-dimensional statistics. For the baseline assumption, it decouples any form of \textit{affectants} in between the quantities, or any form of behaviours of such large enough to get one term to dominate. Listed quantities are normalized on $N/m$, $d/m$, $m/|\mathcal{S}|$, stays $O(1)$ on asymptotic behaviours. This is well-understood and widely-accepted, and we have no issues on such, but only the domain of extension and \textbf{interpretative restriction} - toward where and how the constraints of $N,m,d,|\mathcal{S}|$ are true enough and will they be extensible. While it keeps the dynamic possible and thus reducible into a small-$\delta$ near static version of the bound in fixed regime, those parameters are rather sensitive, and would have structural predicates more than what was assumed in certain cases. The action of having $d\gg |\mathcal{S}|$ or $N\gg m$ incurs certain regimes that might elucidate the problem than not. However, those are structurally valid restriction. The second sub-assumption of such, however, is in active effect of covering for a structural issue. The assumption on $\lim \sup_{|\mathcal{S}|}||\bm{S}||<K_{S}$ on bounding the spectral norm of the feature matrix means assuming a normalized, non-vacuous only in specific region of the normalization convention. This assumption is used to bound the term $||\Sigma_{\mathcal{S}}||/\sqrt{m}$ to be concentrated, but in turns, it directly imports it both in those structural conditions, and also all other results, including the bound on 
\begin{equation}
    \mathrm{Pr}\left(\frac{1}{m}||\Sigma_{X}||\geq t\right) \leq CNe^{-\frac{cm^{2}t^{2}}{2n||\bm{X}||^{2}}}, \quad \forall t \geq 4 t_{0}, t_{0} = |\sigma(0)| + ||\bm{X}||\sqrt{\frac{d}{m}} 
\end{equation}
which is from another paper, which compounds the hidden chain of assumptions in which double descent is very sensitive in. Moving back, the issues contain also the condition that the state matrix has a specific condition in which it must be \textbf{controlled coherent}, meaning the columns are configured in such a way that singular value does not accumulate during operations. The experiments used in the paper, however, also contradicts this by setting $d=50$ fixed for condition one requires $d/m\to \mathrm{const}$, which means the experiment itself at this stage, while tractible, does not amount to much but crude strict-case manufactured double descent. 

Assumption 2 is rather deliberate on conditioning $H(\hat \mA_m)$ on specificity of the empirical transition model matrix $\hat \mA_m$. While not much can be said about this design matrix, nonsymmetric, and is so far not in operator theory application or thereof. Symmmetrization happens when the author needs $Q_{m}(\lambda)$, which means in such $\tfrac{1}{m}(\hat \mU_n - \gamma \hat \mV_n)^T \mSigma_{\hat{\mathcal{S}}}^T \mSigma_{\hat{\mathcal{S}}} \hat \mU_n + \lambda\mI_{n}$ is invertible. The symmetrization as such is derived into the symmetric component $H(\hat \mA_m)$, which is positive definite to get that the eigenvalues of this matrix has nonnegative real parts. Note that positive definiteness of $H(\hat \mA_m)$ is not similar to on strictly $\hat \mA_m$, but only operable via the inequality 
\begin{equation}
    \mathrm{Re}(\nu_{j}(\hat \mA_m)) \geq \nu_{\min}(H(\hat \mA_m)) \geq \xi_{\min}
\end{equation}
for uniform positive definiteness. For a nonsymmetric matrix, it can be highly non-normal, such is why when extracting eigenvalues and parts of the symmetrization, the carving itself potentially removes the actual components and only take on residue of the transient amplification of the mechanics. For example, as the skew component $S(\hat \mA_m)=\frac{1}{2}(\hat \mA_m - \hat{\mA_m}^{\top})$, then $H(\hat \mA_m)\succ 0$ does not guarantee same behaviours for $S(\hat \mA_m)$. The inequality itself bounds only the real part on this projection onto spectral complex, which means the bound is rather extremely weak on this complex space projection of operator Hermitian. Furthermore, it removes also the domain of off-policy learning, function approximation fall-off, by constraining $I-\gamma P$ instability, which perhaps is a simplification choice, but left no way to extend results cleanly toward a larger regime than to completely rederive everything. The assumption is then not self-contained as evidentially provided of, but unstated of structural constraints. 

The third assumption is rather also radical, as it is very hard to justify the jump into reduction of convergence rate, to $\mathcal{O}(1/\sqrt{m})$ from $\mathcal{O}(1/\sqrt{n})$. This implicitly requires $n$ to grow at least as fast as $m$ in such way. The quantity on $\hat{\bm{A}_{n}}$ itself involves the full state space decomposition, not just the operational visited states, and of which ask the norm bound on the matrix that includes structurally zero rows, to approximate nonzero rows, which only is plausible when $m/|\mathcal{S}|\to 1$, where coverage is near-complete. This is because the decomposition to terms $U_{n},V_{n}\in\mathbb{R}^{|\mathcal{S}|\times n}$ requires controlling $|\mathcal{S}|$ for the operator norm bound in the stated condition, of which for unvisite states, the corresponding rows of $U_{n}$ are zero, so their empirical estimate of transition behaviour is exactly zero, while the true proportion on $\mathbf{D}_{\bm{\pi}}\left[\mathbf{I}_{|S|} - \gamma \mathbf{P}\right]$ being nonzero weight assignment. 

There are other assumptions hidden in the setup. Random features here are assigned with Gaussian weights, and the condition also gives system on $K_{\sigma}$-Lipschitz condition and activation. The $\sqrt{n}$ normalization term of which appears everywhere, however not stated of its broader structural roles aside from convenience sake. On-policy sampling is deliverately restricted, however not fully realized of its effect, where such condition actually informs the third assumption itself. In a sense, the paper itself is not of any particular aspect TD learning with double descent identification of structures. Rather, it is a theory of on-policy LSTD with Gaussian random features in a proportional-growth regime with near-complete coverage, which is as similar of methods to the extremely reduced case of linear regression on standard double descent singular matrix collapse. The intrusion of mapping onto the representative structure of the operator theory domain, and more specifically on complex representation is also non-trivial. This jump from a real system to the encoding on Ostrowski lemma for $\bm{A}$ Hermitian, and the proof of several lemmas in the paper, of which invoked $\nu_{\min}(H(\hat{\bm{A}}^{-1}))$ are flowed as if spectral theory for Hermitian operator applies on such system, which on itself constraints a rather very deliverate interpretative spectral domain, but also $\hat{\bm{A}}$ was previously a real asymmetric matrix, and no explanation was provided on such part for the logical transformation of such. In a sense, the paper invokes Hermitian/PSE spectral theory whenever conveninent by symmetrization and truncation toward such regime, and there exists the absence of such clarification on why and what was lost under the transition itself, as also seen by the skew-aspect criticized earlier. In Appendic C, the reformulation requires Mercer's theorem. For such theorem to apply, you require several assumptions: $(i)$, that the compactness is assumed to be consistent before such transition of problem space, $\mathcal{C}$ compact such that $\mathcal{S}\subset \mathcal{C}$ is rather very non-trivial. The import of $L^{2}$ inner products, integral operators, and so on. The Hilbert space $L^{2}(\mathcal{C},\pi)$ is unwarranted and exists only because of the forcing constraints to operare Mercer's tools, and is not forced by the problem. $(ii)$, is that the asymptotic feature space of which has $M$ potentially infinite eigenfunctions $\{\omega_{i}\}$ in the paper, is accompanied by $N$ random features on algorithm. The approximation error from this truncation is not justified or quantified in the derivations. $(iii)$, the operator norm and spectral theory section are Hermitian by convenience, of which none is forced. In a sense, such mapping correlates to kernel machine learning actions where one maps this domain of linearlized system into self-adjoint kernel space, which on itself is the property admitted of the kernel, not other components. And $(iv)$, the stationary distribution $\pi$ is also the measure defining the Hilbert space, but also the definitive factor for MRP and MSBE norm. This transition and whether the coupling makes much sense is not specifically treated during role compoundation, which risks that the analysis is conflating the two simultaneously. 

If we are to push further, we can deliberately say that the paper itself both oversold, and broke its own thesis. For example, the paper's stated purpose is to explain double descent in RL with TD learning scheme, specifically derived toward the complexity $N/m$ term and where it produces a phase transition. However, aside from the term $N/m$ itself being again, another heuristic created in the spatial definition axis without backward compatibility toward other notions of model complexity that operates in conjunction within the model space that the metric is, without justification and without knowing why (which, under specific instance we can say that the phase transition is engineered by the predicted asymptotic ordering, not forced under experimental details), the MSBE being analysed is not consistent of what was used. The true MSBE is defined by 
\begin{equation}
    \widehat{\operatorname{MSBE}}(\vtheta) = \tfrac{1}{n} \lVert \rtrain + \gamma \Sigma^{\top}_{\bm{X}_{n}'} \vtheta - \Sigma^{\top}_{\bm{X}_{n}'}\vtheta \rVert^2,
    \label{def:MSBE_train}
\end{equation}
while parts of the analysis uses the second asymptotic MSBE as 
\begin{equation}
    \operatorname{MSBE}(\vtheta) = \lvert \lvert \bar{\bm{r}} + \gamma \bm{P} \Omega_{\mathcal{S}}^{\top}\bm{v}_{\theta}- \Omega_{\mathcal{S}}^{\top} \bm{v}_{\theta}\rvert \rvert^{2}_{\bm{D}_{\bm{\pi}}} + f_{\Delta} \left(\bm{D}_{\bm{\pi}}^{1/2}(\bm{I}-\gamma\bm{P})\Omega^{\top}_{\mathcal{S}}\right)
\end{equation}
where $\Omega_{\mathcal{S}}$ is the infinite-dimensional eigenfunction feature matrix, $\bm{v}_{\theta}$ is the weight vector in asymptotic feature space with rescaled regularization parameter stated in the paper being $\lambda(m(1+\delta))/N$. These are inherently not the same object, and the double asymptotic equivalence between the two are very fragile, of which is only true for $N,m,d\to \infty$. Under normal finite circumstance, this fails immediately on consequence. The term $f_{\Delta}$ in such derivation is attributed to double descent, but it is a kernal property on the operationalized space, not the finite random feature model. Whether they actually coincide is a problem that is very much open for exploration, but within such length of the paper one would expect this to be seen or at least partially provided, both for this gap and the assumption thereof. The regularization change from the original algorithm of $\lambda = \lambda_{m,n}/mn$ was silently changed also changed the status of the dervied results into informal extrapolation from a formally different structure system, and is not conditioned on anything, which correlates to the same flawed PAC-descent method that we have seen. And lastly, the double descent explanation itself is circular in its own nature. The paper states for once that the correction terms $\delta$ and $\Delta$ are responsible for double descent, however, $\delta$ is derived from deterministic equivalent of the resolvent $Q_{m}(\lambda)$, which are inherited of two separated papers on the structure precondition and phase transition behaviours. $\Delta$ itself being the second-order term involving $\delta$ also makes it so that the structural preconditioning presupposes double descent by directly engineer a space where such is susceptible for the expression used, yet also claimed to elucidate double descent using those, which is a circular problem in its own right. Whether if this is the correct or wrong reading of the paper and its problem, such is remained to be seen from revisions, but under the current status of the paper, such cannot be contained, as it requires tracking structural overcommitment and hidden assumption, proliferation of multiple matrices, Gram/kernel representation, resolvents, regularization parameters, two LSTD solutions, dual-$\pi$ usage and interpretation, and more than thirty different structural predicates in general of the setting, makes the analysis non-trivial on such grounding for little results. 


\subsection{Variations on analysis}
Many sub-variations and analysis alongside major substantiated discoveries listed above can help in analysis. Some of them are toward linking different existing dynamics to large neural network in non-linear fashion, or generalizing approximation of linear network toward nonlinear systems. There are many analyses, typically of interesting being weak feature double descent - \cite{Belkin_2020}, insights on particularly out-of-Distribution Detection - \cite{ammar2025doubledescentmeetsoutofdistribution}, Gaussian processes and neural network - \cite{harzli2023doubledescentcurvesneuralnetworks}, and the proliferation of Grokking, Emergence, and Double Descent - ~\cite{huang2025samplecomplexityrepresentationability}. While interesting, we do not specifically cover them in analysis or the variations, as they exhibited the same structural deficiency or experimental validation patterns not so much contributed to more understanding on such matter beyond what has been said. 
